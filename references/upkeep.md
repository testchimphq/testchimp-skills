...
4. **Environment contract (strict, before planning):** Before starting **Analyze** or authoring the evolve plan, read **`plans/knowledge/policies/connect-to-test-env.policy.md`** (Local Agent / CI Cloud). **Legacy fallback only:** if the policy is missing and **`plans/knowledge/ai-test-instructions.md` already exists**, read **`## Environment Provision Strategy`**. Do **not** create `ai-test-instructions.md` for env. Use that guidance to shape the plan and execution ordering. Re-read the same contract again immediately before any test authoring/execution work, and follow it exactly (no improvised target URLs or provisioning flow).
...
- Call **`get-requirement-coverage`** as a **top-N gap recommender**. Expand **`global.policy.md`**:
  ...
  - **Prioritization signals:** `scenario_priority: true` → `--consider-scenario-priority`; `semantic_coverage: true` → `--consider-semantic-coverage` (packs unlinked scenario gaps by embedding novelty vs linked scenarios).
  - Prefer **`--limit`** (e.g. 20); when omitted, server uses `max_new_tests_per_workflow_execution` from policy if `>0`, else **20**. Always set **`includeNonCoveredUserStories` / `includeNonCoveredTestScenarios`: true**.
  - Prefer response **`rankedScenarios`** as the work queue (eligibility = present there — with semantic on, server returns **unlinked** scenario gaps; without semantic, execution gaps including partial multi-platform `NOT_ATTEMPTED`). Nested `userStories` is for tree views only.
...
- Run **new or changed** tests per **`plans/knowledge/policies/connect-to-test-env.policy.md`** (local vs CI, env bring-up, headed vs headless—follow what the project recorded; legacy FAQ only if `ai-test-instructions.md` already exists—[`run-qa.md`](./run-qa.md#binding-test-env-contract-policy-and-legacy-faq)).
