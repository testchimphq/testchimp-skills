...
**One-time per project** (repo / team): platform comms, folder mapping, test environment, CI wiring, optional imports, smoke validation. Runs in ChimpHands or a local agent workspace. **Not** the per-developer workstation setup — that is [`init-testchimp.md`](./init-testchimp.md) (`/testchimp init`).
...
- **`get-git-folder-mapping`** / **`update-git-folder-mapping`** — read/write mapped `plans/` and `tests/` paths on the platform after agent scaffolds folders in a PR (prefix branches `testchimp-`).

Required for `overall_complete`: `platform_comms`, `folder_mapping`, `connect_to_test_env`, `ci_wiring`. Optional items do not block completion. Mocking is optional — no init-status field.

Also persist decisions in **`plans/knowledge/ai-test-instructions.md`** as today.
