...
Use this as the primary reference for `/testchimp test`. For SmartTest authoring patterns and examples, load **[`write-smarttests.md`](./write-smarttests.md)** during the **Execute** phase.
...
4. **Prerequisites + environment strategy**
   ...
   - Read `plans/knowledge/ai-test-instructions.md` first for persisted project decisions.
...
**Plan output:** write a `.md` plan artifact summarizing scenarios, missing scenarios, test matrix (UI/API/manual), prerequisites, environment choice, and infra-gap actions.
...
4. Run written tests with Playwright and fix failures iteratively.
5. Retry fix-and-rerun up to **2 attempts per failing test**. If still failing, stop retrying those and clearly ask user for help with the unresolved failures (in the report created in the next phase).
