...
6. **TrueCoverage (when applicable)**
  ...
  - **Per-event metadata keys:** These slice **per-event** coverage metrics. Use **only low-cardinality** keys whose values form a **small, meaningful set** for comparing coverage (e.g. coarse **checkout path**, **payment method** category). **Do not** include **PII**. **Do not** use **identifiers** or other **high-cardinality** keys/values (e.g. `project_id`, `org_id`, `user_id`, raw emails, free-text, UUIDs)—they are unsuitable for sliced coverage and explode cardinality. Design keys so each dimension answers: *“Would I want a coverage breakdown by this slice?”*
