...
| **Plan** | Every inventory test row notes **platform** (`web` / `ios` / `android`). Smart smoke **§6** and ExploreChimp **§7** lists are **filtered** to scoped platforms. Coverage/history queries use `--platform` when analyzing one stack ([`run-qa.md`](./run-qa.md) Analyze inputs). |
