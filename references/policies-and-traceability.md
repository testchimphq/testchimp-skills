...
0. **Base branch (cloud / automation prompts)** — If the prompt includes `Base branch: <name>` (legacy: `Working branch:`), that is the **parent** branch — check it out before scoping or planning, then create a **`testchimp-*` agent branch** before any edits (ChimpHands CI: [`chimphands.md`](./chimphands.md)). On **CI / ChimpHands**, use a **shallow single-branch fetch** only (see [`chimphands-ci-runner.md`](./chimphands-ci-runner.md)) — never `git fetch origin`. If `CHIMPHANDS_WORK_BRANCH` or the workflow already checked out the base branch, use it as parent only — do **not** commit there. Do **not** ask the user which branch to use in `--mode=non-interactive` or other non-interactive cloud runs. After checkout of the **default** branch for an **`implement`** workflow, create a **new `testchimp-*` feature branch** before coding (same rule as Raise a PR below).
