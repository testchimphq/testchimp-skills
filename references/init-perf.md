...
4. Run `k6/scripts/prepare.sh` so npm `@testchimp/k6` **latest** lands in gitignored `k6/lib/` (re-fetched on each prepare/run). **Never** pin `K6_REPORTER_VERSION`. **Never** vendor a reporter copy into git. `K6_REPORTER_LOCAL_DIR` is only for unpublished dogfood.
