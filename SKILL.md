...
| `/testchimp fix issue` / `/testchimp fix issue: <id>` | [`references/fix-issue.md`](references/fix-issue.md) — workflow **`fix-issue`**. |
...
| `/testchimp implement` / `/testchimp implement <story/scenario id or plan.md>` | [`references/implement-requirement.md`](references/implement-requirement.md) — workflow **`implement`**; accepts story/scenario **or plan file** (resolve `story:` from plan); Analyze → Plan (adopt plan file / task titles) → Execute (`create-issue` **`TASK_ISSUE`** + story **`linkTargets`** when story known → implement → **`update-issue-status` `FIXED`** per task) → Report (`IMPLEMENTED` + [Report workflow execution](references/policies-and-traceability.md#report-workflow-execution)). |
