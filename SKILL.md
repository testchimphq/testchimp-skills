...
| `/testchimp run security scan for <scan_id>` | [`references/security_scans.md`](references/security_scans.md) — Release Checks security scan: load config, resolve `.env*` `BASE_URL`, ZAP DAST (passive → optional active with user confirm), `report-dast-findings`, `update-scan-progress`. |
...
| [`references/security_scans.md`](references/security_scans.md) | `/testchimp run security scan for <scan_id>`: ZAP DAST, scan progress, report-dast-findings |
