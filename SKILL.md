...
- **Requirement traceability** via Playwright **`annotation`** on tests (e.g. `{ type: 'scenario', description: '#TS-101' }`) linking SmartTests to scenarios. One test may include **multiple** scenario annotations in the same `annotation` array when it covers several scenarios—see [`references/write-smarttests.md`](references/write-smarttests.md). Do **not** author deprecated `// @Scenario:` comments.
...
| [`assets/policies/global.policy.md`](assets/policies/global.policy.md) | Default **global** policy (`policy-kind: global`) — coverage target, prioritization signals, suite size limits, suite **`annotations:`** (e.g. `group` smoke/regression) |
