...
- **Setup QA infra** - sets up opinionated, enterprise-grade QA infra including CI setup, test-only seed / teardown / read endpoints, world-state scripts, mocking strategy, TrueCoverage instrumentation, per-PR environment provisioning.
...
- **World-States** - reusable world-state scripts that call **seed** (and often **teardown**) endpoints to bring environments to known pre-defined states; **read** endpoints support assertions on persisted state after UI actions. See [`references/seeding-endpoints.md`](references/seeding-endpoints.md) and [`references/world-states.md`](references/world-states.md).
...
| [`references/seeding-endpoints.md`](references/seeding-endpoints.md) | Test-only seed, teardown, and read endpoints; discovery, proxy pattern, idempotency, world-states |
