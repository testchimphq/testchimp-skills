...
4. **`TESTCHIMP_API_KEY` + `TESTCHIMP_BACKEND_URL` (P0 — single rule for MCP, CLI, and runners)** — Any process that runs **Playwright** or **Mobilewright** with **`@testchimp/playwright`**, or **`testchimp`** CLI against the project APIs, must have **`TESTCHIMP_API_KEY`** in **that process’s** environment. **IDE-only or MCP-only** config does **not** satisfy the **child** test runner or a **CLI fallback** shell. If you **cannot verify** the key is set on the process **before** spawn, **halt** — do not run tests “to see what happens.”
   - **Resolve (never print secrets):** SmartTests root ( **`.testchimp-tests`** ) → walk **up** to **project-level** host MCP config (e.g. **`.cursor/mcp.json`** for Cursor, **`.mcp.json`** at repo root for Claude Code) → read **`mcpServers.testchimp.env`** and **export / inject all of**:
     ...
     3. **`TESTCHIMP_PROJECT_ID`** when present (TrueCoverage RUM `projectId` if not already in app config — [`references/truecoverage.md`](references/truecoverage.md))
...
| `/testchimp fix test failure` | [`references/fix-failing-tests.md`](references/fix-failing-tests.md) — Fetch a raw execution report by `batch_invocation_id` (for multiple tests run in a single batch job) or `job_id` (for an individual test run), troubleshoot, apply fixes, and re-run failing tests per `plans/knowledge/ai-test-instructions.md`. |
| `/testchimp fix issue` / `/testchimp fix issue: <id>` | [`references/fix-issue.md`](references/fix-issue.md) — Fetch issue details by ordinal (`get-issue-details`), fix from description + linked execution/batch + signed artifacts, set `IN_PROGRESS_BUG` after applying code, set `FIXED` only after user confirmation / commits pushed. |
...
| [`references/fix-failing-tests.md`](references/fix-failing-tests.md) | `/testchimp fix test failure`: fetch execution report by batch/job id and fix failing SmartTests |
| [`references/fix-issue.md`](references/fix-issue.md) | `/testchimp fix issue`: fetch issue by ordinal, fix from linked execution/artifacts, update status |
