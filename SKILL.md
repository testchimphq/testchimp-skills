...
compatibility: Requires Node.js; web projects need @playwright/test and playwright >= 1.59.0 (see Preamble checks #6). Mobile projects need mobilewright + @mobilewright/test (see references/mobilewright-smarttests.md). **`TESTCHIMP_API_KEY`:** Preamble checks **#4** (runner process, not only MCP/IDE). Network access for TestChimp APIs when using MCP, CLI, or AI steps.
version: 0.2.15
...
**Minimum versions:** This skill declares **`required_cli_version`** in frontmatter. Agents must run **Preamble checks #5** (CLI) and **#6** (Playwright/Mobilewright toolchain).
...
| `/testchimp test` | [`references/testing-process.md`](references/testing-process.md) — **Preamble #4** before any `@testchimp/playwright` run. **first read `plans/knowledge/ai-test-instructions.md` (Environment Strategy + FAQ)**, then **Analyze → Plan → Execute → Validate → Phase 5 (ExploreChimp) → Phase 6 (Cleanup)** — **full chain** through Phase 5 or branch-plan **`N/A`** when UI SmartTests are in scope (see reference). **Between phases:** complete each **phase completion gate**; every line **done** or **`N/A`** + one-line justification (record on the **branch plan** file). |
...
See [`references/testing-process.md`](references/testing-process.md) for the full `/testchimp test` workflow (**Analyze → Plan → Execute → Validate → Phase 5 (ExploreChimp) → Phase 6 (Cleanup)**) including non-negotiables, required plan sections, **Preamble #4**, and the checklist gating mechanism.
...
| [`references/testing-process.md`](references/testing-process.md) | `/testchimp test` strict workflow: Analyze → Plan → Execute → Validate → Phase 5 (ExploreChimp; default-on for UI test deltas) → Phase 6 (Cleanup); checklist gates; “run tests” vs full-chain semantics |
