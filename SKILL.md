...
| `/testchimp test` | [`references/testing-process.md`](references/testing-process.md) — **Preamble #4** before any `@testchimp/playwright` run. **first read `plans/knowledge/ai-test-instructions.md` (Environment Strategy + FAQ)**, then **Analyze → Plan → Execute → Validate → Phase 5 (Smart regression) → Phase 6 (ExploreChimp) → Phase 7 (Cleanup)** — **full chain** through Phase 6 or branch-plan **`N/A`** when UI SmartTests are in scope (see reference). **Between phases:** complete each **phase completion gate**; every line **done** or **`N/A`** + one-line justification (record on the **branch plan** file). |
