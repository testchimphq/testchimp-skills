...
- **Periodically**, run **`/testchimp upkeep`** (synonym **`/testchimp evolve`**) — "I will" (similar to above say in first-person) analyze requirement coverage gaps and TrueCoverage insights—by communicating with the TestChimp platform—and address them systematically: tests and infra to cover under-tested slices, and when planned, **ExploreChimp** runs on **TrueCoverage-prioritized** UI journeys (drop-offs, duration/demand hotspots) to surface UX issues in those critical areas ([`references/upkeep.md`](references/upkeep.md)).
