...
- **TrueCoverage** - feedback loop for test coverage aligned with real user behaviour insights from production. **Default: opted-in** unless `plans/knowledge/ai-test-instructions.md` **explicitly** records a TrueCoverage opt-out under `### TrueCoverage Plan` (see [`references/truecoverage.md`](references/truecoverage.md)).
- **ExploreChimp (UX analytics on UI journeys)** — Optional local runs with **`EXPLORECHIMP_ENABLED`** and the **`markScreenState`** fixture send DOM, screenshot, console, network, and metrics checkpoints to TestChimp so agents can surface **UX issues** (performance, layout, visual, usability, accessibility, and related signals) **along the same pathways as SmartTests**. Pure API-only automation is out of scope. Workflow: [`references/exploratory_runs.md`](references/exploratory_runs.md); command alias **`/testchimp explore`**; in **`/testchimp test`**, optional **Phase 5** after **Phase 4: Validate** when the branch plan opts **in** ([`references/testing-process.md`](references/testing-process.md)); optional in **`/testchimp evolve`** when **TrueCoverage** signals select high-impact UI slices ([`references/evolve-coverage.md`](references/evolve-coverage.md)).
...
6. **Playwright toolchain check** — TestChimp requires Playwright 1.59.0+. **Before** authoring SmartTests, running **`npx playwright test`**, or doing browser-driven exploration for **`/testchimp init`** smoke, ensure the repo actually has a compliant install:
