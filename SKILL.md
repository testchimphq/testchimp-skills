---
name: testchimp
description: Integrate repositories with TestChimp for QA orchestration — policy-backed workflows, SmartTests, plans, coverage, TrueCoverage, ExploreChimp, and k6 performance testing (init/create/run/upkeep perf). Use for /testchimp commands, SmartTests, policies, performance testing, k6, scenario-linked journeys, perf baselines/comparisons, or skill updates.
...
| `/testchimp init-perf` | [`references/init-perf.md`](references/init-perf.md) — scaffold deterministic k6 assets + `run-perf-tests` policy under SmartTests root; soft-gate `PERFORMANCE_TESTING`. |
| `/testchimp run-perf-tests` / `/testchimp run perf` | [`references/run-perf-tests.md`](references/run-perf-tests.md) — policy-backed, script-first Analyze → Plan → approval → Execute → Validate → Report; related selection and baseline comparison. |
