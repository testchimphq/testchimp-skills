...
**`@testchimp/cli`** is not part of this skill tree. Install it in the app repo (**`@testchimp/cli@latest`**, or pin ≥ **`required_cli_version`** in **`SKILL.md`** frontmatter — currently **0.1.6** for per-platform coverage and scenario execution history). Ask user to configure **`TESTCHIMP_API_KEY`** in the env block of mcp.json (have a placeholder so that it is clear where to update). Use **`npx`** with **`@testchimp/cli@latest`** in **`args`** so **`npx`** picks up new releases. Pair with **`@testchimp/playwright` ≥ 0.2.0** for execution device context on ingest. Sample: **`assets/sample-mcp.json`**. Full steps: **`references/init-testchimp.md`**. Agents verify CLI compatibility via **Preamble checks** in **`SKILL.md`**.
